...
const naruto = require("naruto-name-generator");
...
const getCsvData = async () => {
  try {
    ...
    writer.on("error", (error) => {
      console.error("Error:", error);
      res.status(500).send({ error: "There was an error saving the file." });
    });
  } catch (error) {
    console.error("Error:", error);
    res.status(500).send({ error: "There was an error fetching data from the API." });
...
mongoose.connect("mongodb://localhost:27017/289-AH", { useNewUrlParser: true });
...
function update() {
  ...
  if (d.getTime() - lastUpdate.getTime() >= 86400000) {
    ...
    getCsvData();
    console.log("get the csv from the server");
    //drop database
